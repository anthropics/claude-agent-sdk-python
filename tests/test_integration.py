...
class TestIntegration:
    ...
    def test_structured_output(self):
        """Test query with structured output schema."""
        ...
        async def _test():
            with patch(
                ...
                # Mock the message stream with structured output
                async def mock_receive():
                    yield {
                        "type": "assistant",
                        "message": {
                ...
                schema = {
                    ...
                    "required": ["file_count", "total_lines"],
                }
                ...
                # Check result message has structured output
                assert isinstance(messages[1], ResultMessage)
                assert messages[1].subtype == "success"
                assert messages[1].structured_output is not None
                assert messages[1].structured_output["file_count"] == 10
                assert messages[1].structured_output["total_lines"] == 523
