...
class TestSubprocessBuffering:
    ...
    def test_whitespace_at_chunk_boundary_preserved(self, ws: str) -> None:
        """Whitespace inside a JSON string value that falls on the boundary
        between two TextReceiveStream chunks must not be stripped.

        Regression: the reader used to call ``.strip()`` on each chunk before
        accumulating it, which silently dropped whitespace from the interior of
        a string value whenever anyio's 64KiB read boundary landed on it. In
        the wild this showed up as a single space missing from a large (>60KB)
        tool-call argument, with no error anywhere.

        ``JSON.stringify`` on the CLI side escapes tab/newline/CR but passes
        U+0020 and Unicode spaces (NBSP, U+2009, U+3000) through raw, and
        Python's ``str.strip()`` eats all of them.
        """
    ...
    def test_whitespace_preserved_across_realistic_64kib_chunking(self) -> None:
        """End-to-end shape of the original report: prose well over 64KiB, cut
        into chunks at exactly the boundary anyio's asyncio backend uses."""
    ...
    def test_complete_oversized_line_still_raises(self) -> None:
        """The buffer-size guard must bound a whole message, not just the tail
        left over after complete lines are split off.

        Regression: framing lines first made it possible for an oversized line
        that arrived with its trailing newline to be parsed and yielded without
        the limit ever being consulted.
        """
        limit = 512
    ...
    def test_malformed_complete_line_raises(self) -> None:
        ...
        line framing means no later data can complete it — so it must surface
        as CLIJSONDecodeError rather than vanish.
        """
