...
class TestHookEventCallbacks:
    ...
    @pytest.mark.asyncio
    async def test_setup_hook_callback(self):
        """Test that a Setup hook callback receives correct input and returns output."""
        hook_calls: list[dict[str, Any]] = []

        async def setup_hook(
            input_data: HookInput, tool_use_id: str | None, context: HookContext
        ) -> HookJSONOutput:
            hook_calls.append({"input": input_data})
            return {
                "hookSpecificOutput": {
                    "hookEventName": "Setup",
                    "additionalContext": "Setup complete",
                }
            }
        ...
        callback_id = "test_setup_hook"
        query.hook_callbacks[callback_id] = setup_hook
        ...
        assert len(hook_calls) == 1
        assert hook_calls[0]["input"]["hook_event_name"] == "Setup"
        assert hook_calls[0]["input"]["trigger"] == "init"

        response_data = json.loads(transport.written_messages[-1])
        result = response_data["response"]["response"]
        assert result["hookSpecificOutput"]["hookEventName"] == "Setup"
    ...
    @pytest.mark.asyncio
    async def test_session_start_hook_callback(self):
        """Test that a SessionStart hook callback works correctly."""
        ...
        callback_id = "test_session_start_hook"
        query.hook_callbacks[callback_id] = session_start_hook
        ...
        assert result["hookSpecificOutput"]["additionalContext"] == "Welcome back!"

    @pytest.mark.asyncio
    async def test_session_end_hook_callback(self):
        """Test that a SessionEnd hook callback works correctly."""
        ...
        callback_id = "test_session_end_hook"
        query.hook_callbacks[callback_id] = session_end_hook

        request = {
            "type": "control_request",
            "request_id": "test-session-end",
            "request": {
                "subtype": "hook_callback",
                "callback_id": callback_id,
                "input": {
